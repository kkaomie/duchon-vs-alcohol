window.initGame = function initGame(level) {
        ...
        if (level === 99) {
            ...
            if (window.speciModifiers) {
                ...
                // Create 3-wave speci level configuration
                const startingWaveDuration = window.speciModifiers.startingWaveDuration * 1000; // Convert to milliseconds
                ...
                levelConfig.waves = [
                    {
                        id: 0,
                        name: 'Starting Wave',
                        duration: startingWaveDuration,
                        enemies: {
                            ealc1: { spawnChance: 0, spawnInterval: 999999 }, // No spawning
                            ealc2: { spawnChance: 0, spawnInterval: 999999 },
                            ealc3: { spawnChance: 0, spawnInterval: 999999 }
                        },
                        startDelay: 0
                    },
                    {
                        ...
                        enemies: {
                            ealc1: {
                                spawnChance: 1.0, // 100% spawn chance
                                spawnInterval: 8000 / timeSpeed // Every 8s, adjusted by timeSpeed
                            },
                            ealc2: { spawnChance: 0, spawnInterval: 999999 },
                            ealc3: { spawnChance: 0, spawnInterval: 999999 }
                        },
                    ...
                    },
                    {
                        ...
                        enemies: {
                            ealc1: {
                                spawnChance: 1.0,
                                spawnInterval: 5000 / timeSpeed // Every 5s
                            },
                            ealc2: {
                                spawnChance: 1.0,
                                spawnInterval: 8000 / timeSpeed // Every 8s
                            },
                            ealc3: { spawnChance: 0, spawnInterval: 999999 }
                        },
                        startDelay: 0
                    },
                    {
                        ...
                        enemies: {
                            ealc1: {
                                spawnChance: 1.0,
                                spawnInterval: 4000 / timeSpeed // Every 4s
                            },
                            ealc2: {
                                spawnChance: 1.0,
                                spawnInterval: 6000 / timeSpeed // Every 6s
                            },
                            ealc3: {
                                spawnChance: 1.0,
                                spawnInterval: 9000 / timeSpeed // Every 9s
                            }
                        },
                        startDelay: 0
                    }
        ...
        function getSpeciEnemyTypesToSpawn(waveEnemyConfig, lastSpawnTimes, currentTime) {
            ...
            return enemiesToSpawn.length > 0 ? enemiesToSpawn : null;
        }
        ...
        function playLevel(levelData) {
            ...
            function startGame() {
                ...
                if (level === 99 && levelData.speciModifiers) {
                    ...
                    // Create modified copies of all enemy types based on slider values
                    Object.keys(ENEMY_TYPES).forEach(enemyTypeKey => {
                        const modifiedEnemyType = JSON.parse(JSON.stringify(ENEMY_TYPES[enemyTypeKey]));
                        ...
                        modifiedEnemyType.damage *= levelData.speciModifiers.enemyDamage;
                        modifiedEnemyTypes[enemyTypeKey] = modifiedEnemyType;
                    });
                ...
                function gameLoop() {
                      ...
                      if (!gamePaused) {
                          ...
                          if (waveEnemyConfig) {
                              const enemyTypesToSpawn = level === 99
                          ...
                          finalWaveSpawnList.forEach((enemyTypeKey, index) => {
                              setTimeout(() => {
                                  if (!gamePaused) {
                                      const randomRow = Math.floor(Math.random() * UNLOCKED_ROWS);
                                      // Use modified enemy types if in speci level, otherwise use original
                                      const enemyType = level === 99 && modifiedEnemyTypes[enemyTypeKey]
                                          ? modifiedEnemyTypes[enemyTypeKey]
                      ...
                      if (gameOver) {
                          ...
                          if (level === 99) {
                              ...
                              drawWrappedText('odomkol si Orechoňa, nuž hlavne Kosačkona, ktorý zachráni riadok raz za hru keď naňho klikneš', canvas.width / 2, canvas.height / 2 - 80, canvas.width * 0.75, 30);
                              ...
                              drawWrappedText('odomkol si chlopatoňa (nie actually rastlina ale buď ticho) (mám rád malé detičky)', canvas.width / 2, canvas.height / 2 - 80, canvas.width * 0.75, 30);
